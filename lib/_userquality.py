...
class UserProjectionMetrics(UserProjectionMetricsProps):
    ...
    def worker(self):
        for self.tile in self.get_get_tiles():
            ...
            try:
                metrics_result = \
                    self.seen_tiles_metric[self.proj][self.name][self.tiling][self.quality][self.user][self.chunk][
                        self.tile]
        ...
        for _ in loop_video_tiling_user():
            ...
            for self.quality in self.quality_list:
                for self.chunk in self.chunk_list:
                    ...
                    try:
                        seen_tiles_metric = \
                            self.seen_tiles_metric[self.proj][self.name][self.tiling][self.quality][self.user][
                                self.chunk]
                    except (KeyError, AttributeError):
...
class ViewportPSNRProps(GetTilesProps):
    ...
    def mount_frame(self, proj_frame, tiles_list, quality):
        ...
        for self.tile in tiles_list:
            ...
            tile_y, tile_x = self.projection_obj.tile_shape[-2] * n, self.projection_obj.tile_shape[-1] * m
            # tile_frame = cv.cvtColor(tile_frame, cv.COLOR_BGR2YUV)[:, :, 0]
            proj_frame[tile_y:tile_y + self.projection_obj.tile_shape[-2],
            tile_x:tile_x + self.projection_obj.tile_shape[-1], :] = tile_frame
