...
class DectimePaths(MakeDecodablePaths):
    @property
    def chunk_dectime_folder(self) -> Path:
        return self.dectime_folder / self.folder_name_proj_tiling_tile_qlt

    @property
    def dectime_log(self) -> Path:
        chunk = int(self.chunk)
        return self.chunk_dectime_folder / f'chunk{chunk:03d}_dectime.log'

    @property
    def dectime_result_by_name(self) -> Path:
        return self.dectime_folder / f'dectime_{self.name}_{self.projection}_{self.rate_control}.pickle'

    @property
    def dectime_result_deprecated(self) -> Path:
